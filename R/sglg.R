...
#' @examples
#' rows <- 210 # Number of observations
...
#' fit1 <- sglg(y1 ~ x1 + x2 - 1, npc=x3, data=data.example)
#'
#' @import ssym
...
sglg = function(formula, npc, basis, data, shape, Tolerance,
    ...
    U_sl <- function(bet, g, sigm, lambd) {
        output <- c(U_sigma(bet, g, sigm, lambd), U_lambda(bet,
            g, sigm, lambd))
        return(output)
    }

    U_gammas <- function(bet, g, sigm, lambd, alph) {
        output <- (-1/(sigm * lambd)) * t(N) %*% W(bet, g, sigm,
            lambd) %*% Ones - M_bar(alph) %*% c(bet, g)
        return(output)
    }
    ...
    loglikglg <- function(bet, g, sigm, lambd, alph) {
        ...
        part1 <- n * log(c_l(lambd)/sigm) + (1/lambd) * t(Ones) %*%
            epsilon - (1/lambd^2) * t(Ones) %*% D(epsilon, lambd) %*%
            Ones
        part2 <- -0.5 * t(c(bet, g)) %*% M_bar(alph) %*% c(bet,
            g)
        output <- part1 + part2
        return(output)
    }
    ...
    newpar <- function(bet, g, sigm, lambd, alph) {
        ...
        output[, 2] <- output[, 1] + solve(I_theta(sigm, lambd,
            alph)) %*% U_theta(bet, g, sigm, lambd, alph)
        condition <- loglikglg(output[1:p, 2], output[(p + 1):(p +
            Tknot), 2], output[p + Tknot + 1, 2], output[p +
            Tknot + 2, 2], alph) - llglg
        if (condition > 0) {
            new <- output[, 2]
        }
        l <- 2
        while (condition > Tolerance & l < Maxiter) {
            ...
            output[, l] <- output[, (l - 1)] + solve(I_theta(output[(p +
                Tknot + 1), (l - 1)], output[(p + Tknot + 2),
                (l - 1)], alph)) %*% U_theta(output[1:p, (l -
                1)], output[(p + 1):(p + Tknot), (l - 1)], output[(p +
                Tknot + 1), (l - 1)], output[(p + Tknot + 2),
                (l - 1)], alph)
            llglg <- loglikglg(new[1:p], new[(p + 1):(p + Tknot)],
                new[p + Tknot + 1], new[p + Tknot + 2], alph)
            condition <- loglikglg(output[1:p, l], output[(p +
                1):(p + Tknot), l], output[p + Tknot + 1, l],
                output[p + Tknot + 2, l], alph) - llglg
            if (condition > 0) {
                new <- output[, l]
                llglg <- loglikglg(new[1:p], new[(p + 1):(p +
                  Tknot)], new[p + Tknot + 1], new[p + Tknot +
                  2], alph)
            }
        }
    ...
    edf <- function(sigm, alph) {
        ...
        output <- diag(1, p + sum(Knot)) + (sigm^2) * inv_root_tNN %*%
            M_bar(alph) %*% inv_root_tNN
    ...
    total_optimum <- function(start) {
        ...
        aic <- output3$AIC
        bic <- output3$BIC
        scores <- U_theta(output[1:p], output[(p + 1):(p + Tknot)],
            output[p + sum(Knot) + 1], output[p + Tknot + 2],
            output1)
        covar <- I_theta(output[p + Tknot + 1], output[p + Tknot +
            2], output1)
        ...
        ordresidual <- eps(output[1:p], output[(p + 1):(p +
            Tknot)], output[p + Tknot + 1])
        sgn <- sign(y - y_est)
        dev <- sgn * sqrt(2) * ((1/output[p + Tknot + 2]^2) *
            exp(output[p + Tknot + 2] * ordresidual) - (1/output[p +
            Tknot + 2]) * ordresidual - (1/output[p + Tknot +
            2])^2)^(0.5)
        Devian <- sum(dev^2)

        good_fit <- gfit(ordresidual, output[p + Tknot + 2])
        part2 <- ((output[p + Tknot + 1])/(output[p + Tknot +
            2])) * (digamma((1/output[p + Tknot + 2])^2) - log((1/output[p +
            Tknot + 2])^2))
        y_est2 <- y_est + part2

        return(list(formula = formula, npc = npc, size = n,
            mu = output[1:(p + Tknot)], sigma = output[p + Tknot +
                1], lambda = output[p + Tknot + 2], y = y, X = X,
            p = p, N = N, Knot = Knot, rord = ordresidual, rdev = dev,
            interval = inter, llglg = llglg, AIC = output2,
            BIC = output3$BIC, scores = scores, Itheta = covar,
            scovar = scovar, st_error = ste, Z_values = zs,
            p.values = pval, alpha = output1, d.f.model = df,
            d.f.npc = dfnpc, deviance = Devian, goodnessoffit = good_fit,
            convergence = output3$Conv, condition = output3$cond,
            iterations = output3$iter, semi = TRUE, censored = FALSE,
            y_est2 = y_est))
    }
