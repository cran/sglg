...
glg = function(formula, data, shape, Tolerance, Maxiter) {
    ...
    I_13 = function(sigm, lambd) {
        output = (-1/(sigm * lambd)) * u_lambda(lambd) * 
            t(X) %*% One
        return(output)
    }
    ...
    U_lambda = function(bet, sigm, lambd) {
        ...
        output = n * eta_lambd - (invlamb^2) * t(One) %*% 
            epsilons + (2 * invlamb^3) * t(One) %*% Ds %*% 
            One - (invlamb^2) * t(One) %*% Ds %*% epsilons
        return(output)
    }
    ...
    loglikglg = function(bet, sigm, lambd) {
        ...
        output = n * log(c_l(lambd)/sigm) + (1/lambd) * t(One) %*% 
            epsilon - (1/lambd^2) * t(One) %*% D(epsilon, 
            lambd) %*% One
        return(output)
    }
    ...
    # Emp_quantil = qnorm(Fs) diff <- qqnorm(Emp_quantil,
    # main = 'Overall goodness-of-fit statistic', cex = 0.3,
    # lwd = 3, xlab = 'Quantiles of N(0,1)', ylab = 'Overall
    # residuals') abline(0, 1, col = 2) output =
    # mean(abs(diff$x - diff$y)) return(output) }
    ...
    optimum = function(bet, sigm, lambd) {
        ...
        llglg = loglikglg(new[1:p, l], new[(p + 1), l], new[(p + 
            2), l])
        condition = llglg - loglikglg(new[1:p, 1], new[p + 
            1, 1], new[p + 2, 1])
        if (condition > 0) {
            output = new[, l]
        }
        while (condition > Tolerance & l < Maxiter) {
            l = l + 1
            new[, l] = newpar(new[1:p, (l - 1)], new[(p + 
                1), (l - 1)], new[(p + 2), (l - 1)])
    ...
    if (output$conv == TRUE) {
        ...
        output = list(formula = formula, size = n, mu = output[1:p], 
            sigma = output[p + 1], lambda = output[p + 2], 
            y = y, p = p, X = X, Knot = 0, llglg = llglg, 
            scores = scores, AIC = aic, BIC = bic, AIC2 = aic2, 
            deviance = devian, rdev = dev, Itheta = covar, 
            st_error = ste, z_values = zs, p.values = pval, 
            interval = inter, goodnessoffit = good_fit, convergence = conv, 
            condition = condition, iterations = iter, y_est = y_est, 
            rord = ordresidual, semi = FALSE, censored = FALSE)
        class(output) = "sglg"
        return(output)
    }
