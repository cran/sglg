...
#' fit3 <- survglg(Surv(log(obst1),delta1) ~ x1 + x2,data=data.example)
#' @import Formula
#' @import survival
#' @import ssym
#' @import methods
...
survglg = function(formula, data, shape, Maxiter, Tolerance) {
    ...
    S = function(y, lambd) {
        s = pgamma((1/lambd^2) * exp(lambd * y), 1/lambd^2, 
            lower.tail = FALSE)
        return(s)
    }
    ...
    loglikglg = function(bet, sigm, lambd) {
        epsil = eps(bet, sigm)
        output = sum(Delta * log(S(epsil, lambd)) + (1 - 
            Delta) * (log(c_l(lambd)/sigm) + (1/lambd) * 
            epsil - (1/lambd^2) * exp(lambd * epsil)))
        return(output)
    }
    ...
    newpar = function(bet, sigm, lambd) {
        ...
        output[, l] = output[, (l - 1)] - solve(I_tetha(output[1:p, 
            ...
            U_theta(output[1:p, (l - 1)], output[(p + 1), 
                (l - 1)], lambd)
        llglg = loglikglg(output[1:p, 2], output[p + 1, 2], 
            lambd)
        condition = llglg - loglikglg(new[1:p], new[p + 1], 
            lambd)
    ...
    gfit = function(resid, lambd) {
        ekm = survival::survfit(Surv(exp(resid), 1 - Delta) ~ 
            1)
    ...
    if (output$conv == TRUE) {
        ...
        output = list(formula = formula, size = n, per.cens = per.censo, 
            p = p, mu = output[1:p], sigma = output[p + 1], 
            lambda = lambda0, y = Y[, 1], delta = Delta, 
            X_bar = X, y_est = y_est, rord = ordresidual, 
            rdev = dev, deviance = devian, Itheta = scovar, 
            scores = scores, goodnessoffit = good_fit, llglg = llglg, 
            AIC = aic, BIC = bic, AIC2 = aic2, st_error = ste, 
            z_values = zs, p.values = pval, interval = inter, 
            convergence = conv, condition = condition, Iterations = iter, 
            semi = FALSE, censored = TRUE)
        class(output) = "sglg"
        return(output)
    }
