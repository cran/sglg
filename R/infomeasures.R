#' Tool to calculate information measures in the original scale of the response variable, such as, AIC and BIC.
#'
...
#' @param fit an object of the class sglg. This object is returned from the call to glg(), sglg(), survglg() or ssurvglg() function.
...
#' infomeasures(fit3)
#' @export infomeasures
infomeasures <- function(fit) {
  if (missingArg(fit))
     print('There is no an object of class sglg.')

  AIC2 <- fit$AIC + 2 * sum(fit$y)
  BIC2 <- fit$BIC + 2 * sum(fit$y)
  output <- list(Original_AIC=round(AIC2, 2),Original_BIC=round(BIC2, 2))
  return(output)
}
